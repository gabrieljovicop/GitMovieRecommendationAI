...
#     user_movie_ratings = pd.concat(
#         [user_movie_ratings, user_movie_ratings.count().to_frame().T],
#         ignore_index=True
#     )
#     # 2- sort
...
def get_most_rated_movies(user_movie_ratings, max_number_of_movies):
    ...
    return user_movie_ratings_sorted.iloc[:, :max_number_of_movies]
...
def get_users_who_rate_the_most(most_rated_movies, max_number_of_users):
    import pandas as pd
    # 1- Count: Tambahkan kolom 'counts' dengan jumlah rating (non-NA) untuk setiap pengguna
    most_rated_movies['counts'] = most_rated_movies.notna().sum(axis=1)
    
    # 2- Sort: Urutkan berdasarkan 'counts' secara menurun
    most_rated_movies_users = most_rated_movies.sort_values('counts', ascending=False)
    
    # 3- Slice: Pilih baris berdasarkan jumlah pengguna terbanyak
    most_rated_movies_users_selection = most_rated_movies_users.iloc[:max_number_of_users, :]
    
    # 4- Drop: Hapus kolom 'counts' setelah selesai digunakan
    most_rated_movies_users_selection = most_rated_movies_users_selection.drop(columns=['counts'])
